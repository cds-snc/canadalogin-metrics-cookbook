---
title: "Data Catalog Template"
# draft: true
---

# Table Name

A straightforward one or two sentence line explaining what the dataset is. Dataset is one word.
...
* **Last Updated**: (if manual) Date/time of last time dataset was updated
...
A bulleted list of field names must be included, alongside a brief description of the field. Boolean descriptions can simply be the question answered by the boolean. The data type of a field and the unit of measurement should be included as well. The names of data types are dictated by the storage format. For example, the Parquet storage format commonly includes booleans, dates, floats, integers, strings, times, and timestamps. There is no need to include the integer or float width unless the circumstances are exceptional.
...
| registration_date | timestamp | When the user registered, in UTC |
